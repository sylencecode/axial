...
module Axial
  module Handlers
    class BindHandler
      ...
      def dispatch_server_disconnect_binds()
        @binds.select { |bind| bind[:type] == :server_disconnect }.each do |bind|
          Thread.new do
            begin
              if (bind[:object].respond_to?(bind[:method]))
                if (bind.key?(:args) && bind[:args].any?)
      ...
      def dispatch_server_connect_binds()
        @binds.select { |bind| bind[:type] == :server_connect }.each do |bind|
          Thread.new do
            begin
              if (bind[:object].respond_to?(bind[:method]))
                if (bind.key?(:args) && bind[:args].any?)
